...
pub const PUZZLE: Puzzle = Puzzle { name: "day7_2" };
...
impl aoc_common::Puzzle for Puzzle {
    ...
    fn get_result(&self, lines: Vec<String>) -> String {
        ...
        let total_size = fs.dirs[0].get_size(&fs);
        let free_space = FS_MAX - total_size;
        let required_space = UPDATE_SIZE - free_space;
        let mut smallest_dir_size: usize = FS_MAX;
        for dir in &fs.dirs {
            let dir_size = dir.get_size(&fs);
            if dir_size >= required_space && dir_size < smallest_dir_size {
                smallest_dir_size = dir_size;
            }
        }

        smallest_dir_size.to_string()
    }
}
